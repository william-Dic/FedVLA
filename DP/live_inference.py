...
def run_live_inference(args, mc: MyCobot):
    ...
    # --- Load Checkpoint ---
    # (Checkpoint loading logic remains the same as before)
    ...
    # (Diffusion setup remains the same)
    try:
    ...
    # --- Image Transform ---
    # (Image transform remains the same)
        ...
        with torch.no_grad():
            while task_loop_active and frame_count < args.max_steps:
                ...
                try:
                    ...
                    frame_count += 1 # Increment even if skipped to avoid infinite loop on bad frames
                    continue
...
if __name__ == "__main__":
    ...
    # (Fallback arguments remain the same)
